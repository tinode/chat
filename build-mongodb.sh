...
GOSRC=${GOPATH}/src/mgi.vn/chat-service
  ...
  do
    ...
    do
      ...
      if [ "$plat" = "windows" ]; then
        ...
        rm -rf ${GOSRC}/releases/build
        cp -rf ${GOSRC}/releases/tmp ${GOSRC}/releases/build

        # Remove possibly existing archive.
        # rm -f ./releases/${version}/tinode-${dbtag}."${plat2}-${arc}".tar.gz
        # Generate a new one
        # tar -C ${GOSRC}/releases/tmp -zcf ./releases/${version}/tinode-${dbtag}."${plat2}-${arc}".tar.gz .
      fi
    done
  done
done
...
cp ${GOSRC}/tn-cli/requirements.txt ./releases/tmp
