...
type topicUnreg struct {
	...
	topic string
	// Session making the request, coudl be nil
	sess *Session
	// Original request, could be nil
	msg *ClientComMessage
	// True if request is generated by an unsibscribed session
	unsubbed bool
...
func (h *Hub) run() {
	...
	for {
		...
		case unreg := <-h.unreg:
			// Topic is garbage collected or being deleted
			h.topicUnreg(unreg.sess, unreg.topic, unreg.msg, unreg.unsubbed, unreg.del)
...
//
// 1. If topic is online and the requester is subscribed, the topic handles the shutdown first:
// 1.1 Topic checks if requester is the owner
// 1.2 Topic evicts all sessions
// 1.3 Topic asks hub to unregister
// 1.4 Topic exits the run() loop
// 1.5 Hub unregisters the topic
// 1.6 Hub deletes the topic from database
// 1.7 Hub informs the origin of success or failure
//
// 2. If topic is online but the requester is not subscribed, forward request to topic
//
// 3. If topic is offline
// 3.1 Hub checks if requester is the owner
// 3.2 Hub deletes topic from database
// 3.3 Hub informs the origin of success or failure
//
// 4. If the topic just wants to unregister, unregister it with no further action
func (h *Hub) topicUnreg(sess *Session, topic string, msg *ClientComMessage, unsubbed, del bool) {
	...
	if t := h.topicGet(topic); t != nil {
		...
		if sub, err := store.Subs.Get(topic, sess.uid); err != nil {
			...
			simpleByteSender(sess.send, ErrPermissionDenied(msg.Del.Id, msg.Del.Topic, now))
			return
		}
	}

	if del {
		// Cases 1.6, 1.7, 3.2, 3.3
