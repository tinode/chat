...
// DecodeAcs parses AccessMode from a byte array.
func DecodeAcs(b []byte) (AccessMode, error) {
	m0 := ModeUnset
...
func (m *AccessMode) ApplyDelta(delta string) error {
	...
	if delta == "" || (delta[0] != '+' && delta[0] != '-') {
		return errors.New("AccessMode: invalid acs delta string '" + delta + "'")
	}
	...
	for i := 0; i < len(delta); i++ {
		...
		case '-':
			if acceptAction {
				actions = append(actions, delta[i:i+1])
				if j != -1 {
					// [j..i) is the last acs delta
					acsDeltas = append(acsDeltas, delta[j:i])
				}
				j = -1
				acceptAction = false
			} else {
				return errors.New("Invalid acs delta string: repetitive or missing action '" + delta + "'")
			}
		default:
			acceptAction = true
			if j == -1 {
				// Start of the new acs delta.
				j = i
			}
		}
	}
	if j != -1 {
		// Last piece.
		acsDeltas = append(acsDeltas, delta[j:len(delta)])
	}
	if len(actions) != len(acsDeltas) {
		return errors.New("Malformed acs delta string: '" + delta + "'")
	}
	m0 := *m
	// Apply updates one by one.
	for i := 0; i < len(actions); i++ {
		if upd, err := DecodeAcs([]byte(acsDeltas[i])); err == nil {
			switch actions[i] {
			case "+":
				if upd != ModeUnset {
					m0 |= upd & ModeBitmask
