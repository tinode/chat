...
func Test_checkAPIKey(t *testing.T) {
	salt := "chdehJymr88WySFXZ/zb5RuayKshiINH7+9K6NkgcMc="
	apikey := generate(100000, 0, salt)
	validate(apikey, salt)
	...
	if declen := base64.URLEncoding.DecodedLen(len(apikey)); declen != APIKEY_LENGTH {
		log.Printf("Error: Invalid key length %d, expecting %d\n", declen, APIKEY_LENGTH)
		return
	}
	if data[0] != 1 {
	...
	hmacSalt, err := base64.URLEncoding.DecodeString(salt)
	if err != nil {
...
func generate(sequence, isRoot int, hmacSaltB64 string) string {
