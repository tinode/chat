...
func (a *adapter) messageGetQuery(projection, topic string, forUser t.Uid, opts *t.QueryOpt) (*sqlx.Rows, error) {
...
func (a *adapter) MessageGetAll(topic string, forUser t.Uid, asChan bool, opts *t.QueryOpt) ([]t.Message, error) {
	...
	if opts != nil {
		...
		opts.IfModifiedSince = nil // Ignored for MessageGetAll
	}
...
func messageDeleteList(tx *sqlx.Tx, topic string, toDel *t.DelMessage) error {
	...
	if toDel == nil {
		...
		// reactions are deleted because of ON DELETE CASCADE
		// filemsglinks are deleted because of ON DELETE CASCADE
		return err
	}
	...
	if toDel.DeletedFor == "" {
		...
		// Delete reactions for these messages. First arg is outer topic, the rest are inner WHERE args.
		delArgs := append([]any{topic}, args...)
		_, err = tx.Exec("DELETE FROM reactions WHERE topic=? AND seqid IN ("+
			"SELECT seqid FROM messages AS m WHERE "+where+")", delArgs...)
		if err != nil {
			return err
		}
...
func (a *adapter) ReactionDelete(topic string, seqid int, userid t.Uid) error {
	...
	// Update message UpdatedAt
	_, err = tx.ExecContext(ctx, "UPDATE messages SET updatedat=? WHERE topic=? AND seqid=?", time.Now(), topic, seqid)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func (a *adapter) reactionsForSet(topic string, forUser t.Uid, asChan bool, opts *t.QueryOpt, seqIds []int) (map[int][]t.OneTypeReaction, error) {
	...
	if len(seqIds) == 0 {
		// No explicit list of seqIds provided. Fetch them according to the options.
	...
	var reactions map[int][]t.OneTypeReaction = make(map[int][]t.OneTypeReaction)
	if asChan {
		for rows.Next() {
			var seqId int
			var content string
		...
		defer userReacts.Close()

		for userReacts.Next() {
			var seqId int
			var content string
			if err = userReacts.Scan(&seqId, &content); err != nil {
				return nil, err
			}
