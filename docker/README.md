# Using Docker to run MidnightChat

All images are available at https://hub.docker.com/r/MidnightChat/
...
2. Create a bridge network. It's used to connect MidnightChat container with the database container.
	```
	$ docker network create MidnightChat-net
	```

3. Decide which database backend you want to use: RethinkDB, MySQL or MongoDB. Run the selected database container, attaching it to `MidnightChat-net` network:

	1. **RethinkDB**: If you've decided to use RethinkDB backend, run the official RethinkDB Docker container:
	```
	$ docker run --name rethinkdb --network MidnightChat-net --restart always -d rethinkdb:2.3
	```
	...
	```
	$ docker run --name mysql --network MidnightChat-net --restart always --env MYSQL_ALLOW_EMPTY_PASSWORD=yes -d mysql:5.7
	```
   ...
   ```
   $ docker run --name mongodb --network MidnightChat-net --restart always -d mongo:latest --replSet "rs0"
...
4. Run the MidnightChat container for the appropriate database:

	1. **RethinkDB**:
	```
	$ docker run -p 6060:6060 -d --name MidnightChat-srv --network MidnightChat-net MidnightChat/MidnightChat-rethinkdb:latest
	```

	2. **MySQL**:
	```
	$ docker run -p 6060:6060 -d --name MidnightChat-srv --network MidnightChat-net MidnightChat/MidnightChat-mysql:latest
	```

	3. **MongoDB**:
	```
	$ docker run -p 6060:6060 -d --name MidnightChat-srv --network MidnightChat-net MidnightChat/MidnightChat-mongodb:latest
	```

	You can also run MidnightChat with the `MidnightChat/MidnightChat` image (which has all of the above DB adapters compiled in). You will need to specify the database adapter via `STORE_USE_ADAPTER` environment variable. E.g. for `mysql`, the command line will look like
	```
	$ docker run -p 6060:6060 -d -e STORE_USE_ADAPTER mysql --name MidnightChat-srv --network MidnightChat-net MidnightChat/MidnightChat:latest
	```
	 ...
	 * [MySQL tags](https://hub.docker.com/r/MidnightChat/MidnightChat-mysql/tags/)
	 * [RethinkDB tags](https://hub.docker.com/r/MidnightChat/MidnightChat-rethink/tags/)
	 * [MongoDB tags](https://hub.docker.com/r/MidnightChat/MidnightChat-mongodb/tags/)
	 * [All bundle tags](https://hub.docker.com/r/MidnightChat/MidnightChat/tags/) (comming soon)
...
The container comes with a built-in config file which can be customized with values from the environment variables (see [Supported environment variables](#supported_environment_variables) below). If changes are extensive it may be more convenient to replace the built-in config file with a custom one. In that case map the config file located on your host (e.g. `/users/jdoe/new_MidnightChat.conf`) to container (e.g. `/MidnightChat.conf`) using [Docker volumes](https://docs.docker.com/storage/volumes/) `--volume /users/jdoe/new_MidnightChat.conf:/MidnightChat.conf` then instruct the container to use the new config `--env EXT_CONFIG=/MidnightChat.conf`:
```
$ docker run -p 6060:6060 -d --name MidnightChat-srv --network MidnightChat-net \
		--volume /users/jdoe/new_MidnightChat.conf:/MidnightChat.conf \
		--env EXT_CONFIG=/MidnightChat.conf \
		MidnightChat/MidnightChat-mysql:latest
```
...
Shut down the MidnightChat container and remove it:
```
$ docker stop MidnightChat-srv && docker rm MidnightChat-srv
```
...
MidnightChat uses Google Firebase Cloud Messaging (FCM) to send pushes.
...
| `CLUSTER_SELF` | string |  | Node name if the server is running in a MidnightChat cluster |
...
| `EXT_STATIC_DIR` | string |  | Path to external directory containing static data (e.g. MidnightChat Webapp files) |
...
| `MYSQL_DSN` | string | `'root@tcp(mysql)/MidnightChat'` | MySQL [DSN](https://github.com/go-sql-driver/mysql#dsn-data-source-name). |
...
| `STORE_USE_ADAPTER` | string |  | DB adapter name (specify with `MidnightChat/MidnightChat` container only) |
...
Container is also available as a part of the MidnightChat docker distribution: `MidnightChat/exporter`.
Run it with

```
$ docker run -p 6222:6222 -d --name MidnightChat-exporter --network MidnightChat-net \
		--env SERVE_FOR=<prometheus|influxdb> \
		--env MidnightChat_ADDR=<MidnightChat metrics endpoint> \
		... <monitoring service specific vars> \
		MidnightChat/exporter:latest
```
...
| `MidnightChat_ADDR` | string | `http://localhost/stats/expvar/` | MidnightChat metrics path |
...
| `INFLUXDB_PUSH_ADDRESS` | string | `https://mon.MidnightChat.co/intake` | InfluxDB backend url |
| `INFLUXDB_AUTH_TOKEN` | string | `` | InfluxDB auth token |
| `PROM_NAMESPACE` | string | `MidnightChat` | Prometheus namespace |
